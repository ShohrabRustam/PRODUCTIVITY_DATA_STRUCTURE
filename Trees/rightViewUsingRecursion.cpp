...
void rightViewUsingRecursion(TreeNode* root, int level, vector<int> &result) {
    if (!root) return;
    ...
    rightViewUsingRecursion(root->right, level + 1, result);
    rightViewUsingRecursion(root->left, level + 1, result);
...
int main(int argc, const char** argv) {
   ...
   cout << endl;
    ...
    vector<int> rightViewResultRecursion;
    rightViewUsingRecursion(root, 0, rightViewResultRecursion);
    cout << "\nRight View of the Tree using Recursion: ";
    for (int val:rightViewResultRecursion)
    {
        cout << val << " ";
    }
