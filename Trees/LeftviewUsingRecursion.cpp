...
void leftViewUsingRecursion(TreeNode* root, int level, vector<int>& result) {
    if (root == NULL) return;
    ...
    leftViewUsingRecursion(root->left, level + 1, result);
    leftViewUsingRecursion(root->right, level + 1, result);
}
...
int main(int argc, const char** argv) {
   ...
   cout << endl;
    ...
    vector<int> leftViewResultRecursion;
    leftViewUsingRecursion(root, 0, leftViewResultRecursion);
    cout << "\nLeft View of the Tree using Recursion: ";
    for (int val : leftViewResultRecursion) {
        cout << val << " ";
    }
    cout << endl;
